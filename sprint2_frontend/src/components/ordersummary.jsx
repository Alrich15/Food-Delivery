// import React, { Component } from 'react';
import OrderService from "../services/OrderService";
import {FaSearch,FaCartPlus,FaRupeeSign } from "react-icons/fa";
import ItemService from '../services/ItemService';

import React, { useEffect, useState } from "react";
// class OrderSummary extends Component{

//   state={
//     orders: [],
//     // items:[],
//     search:"",
// };

const OrderSummary = (props) => {
  const[order,setOrder] = useState({});
  useEffect(()=>{
    // axios.get(`http://localhost:8083/item/byId/${props.match.params.id}`)
    OrderService.viewOrderById(`${props.match.params.id}`).then((res) => {setOrder(res.data);
    },[]);
  },[]);


// componentDidMount(){
//     OrderService.viewOrder().then((res)=> this.setState({orders: res.data})
//     );
//     // ItemService.viewAllItems().then((res)=> this.setState({items: res.data}));
    

// }

// handleOrder = (id) => {
//   OrderService.viewOrderById(id).then((res) => {
//       const orders=this.state.orders.filter((od)=>od.orderId!=id);
//       this.setState({orders:res.data});
//   });
// };
// class OrderCard extends Component {
//   constructor(props) {
//       super(props);
//       this.state = { 
//           orders:[],
//           tempOrders:[],
//           search:"",
//   }}

//   componentDidMount=async ()=>{
//       const response = await OrderService.viewOrder();
//       this.setState({orders:response.data})
//       this.setState({tempOrders:this.state.orders})
//   }

// handleDelete = (id) => {
//     OrderService.removeOrderById(id).then((res) => {
//         const orders=this.state.orders.filter((od)=>od.orderId!=id);
//         this.setState({orders});
//     });
// };

// handleSearch = (event) => {
//     console.log("handleSearch");
//     this.setState({search: event.target.value});
//     const filteredOrders=this.state.orders.filter(
//       (od)=>od.orderStatus.toLowerCase().startsWith(event.target.value.toLowerCase())
//     //   (od)=>od.orderStatus.toLowerCase().startsWith(event.target.value.toLowerCase())
//     );
//     this.setState({orders:filteredOrders});
//   };

//   handleSearch1 = (event) => {
//     console.log("handleSearch");
//     this.setState({search: event.target.value});
//     const filteredOrders=this.state.orders.filter(
//       (od)=>od.orderId.toLowerCase().startsWith(event.target.value.toLowerCase())
//     //   (od)=>od.orderStatus.toLowerCase().startsWith(event.target.value.toLowerCase())
//     );
//     this.setState({orders:filteredOrders});
//   };


    // render(){
        return(
            
          <div className="container mt-4 mx-auto ">
             
            
    
          <div className="d-flex justify-content-end m-4">
                  {/* <FaSearch size="30px" color="dark" className="mt-1 p-1"/> */}
                      <div>
                     
                          <form>
                              {/* <Search/> */}
                        
                              {/* <input className="form-control" type="search" placeholder="Search by Order Status "  value={this.state.search}  onChange={this.handleSearch}/> */}
                              {/* <button className="btn btn-outline-success" type="submit">Search</button> */}
                          </form>
  
                      </div>
                      {/* <FaSearch size="30px" color="dark" className="mt-1 p-1"/> */}
                       <div>
                     
                          <form>
                              {/* <Search/> */}
                              {/* <input className="form-control" type="search" placeholder="Search by Order Id "  value={this.state.search}  onChange={this.handleSearch1}/> */}
                              {/* <button className="btn btn-outline-success" type="submit">Search</button> */}
                          </form>
  
                      </div> 
                      </div>
                      
      
      <div className="row g-3">
       
          
       
         {/* {order.map((order) => ( */}
        
          <div className=" col-sm-12 col-md-6 col-lg-4 card m-2 p-1 mx-auto " >
              {/* {this.state.items.map((item)=>( */}
              <div className="row g-3 m-2">
                <div className="col-md-4">
                  <img src="data:image/png;base64,"
                  className="img-fluid rounded-start" alt="..." height="100" width="100"/>
                </div>
                <div className="col-md-8 ">
                  <div className="card-body">
                    {/* <h5 className="card-title ">Customer Name : {order.cart.customer.address.addressId}</h5> */}
                    <h4 className="card-title bg-warning border border-3 border-dark rounded">Order ID : {order.orderId}</h4>
                    <h5 className="card-text">Order Date: {order.orderDate}</h5>
                    
                    {/* <p className="card-text"><small className="text-muted">Food Cart id: {od.cart.cartId}</small></p> */}
                    
                    {/* <h6 className="card-text bg-danger text-white border border-3 border-dark rounded">Bill Amount:<h5><FaRupeeSign/>{order.cart}</h5></h6> */}
                    
                      {/* <p className="card-text"><small className="text-muted">Food Cart id: {item.itemName}</small></p> */}

                   
                    
                    <h5 className="mt-3">Order Status</h5><hr /><h5 className="mt-1 bg-warning border border-3 border-dark rounded"> {order.orderStatus}</h5>
                    
                  </div>
                </div>
              </div>
              
            </div>
           
          {/* ))}  */}
            
            </div>
            </div>
            


        );
  }
  // };
// };

export default OrderSummary;

// import React, { useEffect, useState } from "react";
// import {
//   Typography,
//   Container,
//   Grid,
//   CardMedia,
//   CardActions,
//   Button,
//   Box,
// } from "@material-ui/core";
// import axios from "axios";
// import { Link } from "react-router-dom";
// import { useSelector, useDispatch } from "react-redux";
// import { getOrderByIdAction } from "../actions/orderAction";

// const theme = {
//   spacing: 8,
// };

// const OrderSummary = (props) => {
//   //const [product, setProduct] = useState({});
//   const dispatch = useDispatch();
//   useEffect(() => {
//     // axios
//     //   .get(`https://fakestoreapi.com/products/${props.match.params.id}`)
//     //   .then((res) => {
//     //     setProduct(res.data);
//     //   }, []);
//     dispatch(getOrderByIdAction(props.match.params.id));
//   }, []);

//   const order = useSelector((state) => state.order);
//   return (
//     <Container>
//       <Box m={3}></Box>
//       {/* {console.log(order.image)} */}
//       <Grid container spacing={3}>
//         <Grid item sm={3}>
//           <CardMedia
//             // image={order.orderId}
//             title={order.orderId}
//             style={{ height: "300px", width: "300px" }}
//           />

//           <CardActions>
//             <Button variant="contained" color="primary">
//               Add to cart
//             </Button>
//             <Link to="/" style={{ textDecoration: "none" }}>
//               <Button variant="contained" color="secondary">
//                 Buy now
//               </Button>
//             </Link>
//           </CardActions>
//         </Grid>
//         <Grid item sm={9}>
//           <Box display="flex">
//             <Typography
//               gutterBottom
//               variant="h5"
//               component="h2"
//               style={{ marginRight: "30px" }}
//             >
//               {order.orderStatus}
//             </Typography>
//             <Typography gutterBottom variant="h5" component="h2">
//               ${order.orderId}
//             </Typography>
//           </Box>
//           <Box textAlign="left">
//             <Typography variant="body2" color="textSecondary">
//               {order.orderDate}
//             </Typography>
//           </Box>
//         </Grid>
//       </Grid>
//     </Container>
//   );
// };

// export default OrderSummary;


// import React, { Component } from 'react';
// import OrderTable from "./ordertable";
// import { NavLink } from 'react-router-dom';
// import OrderService from '../service/orderService';
// import {ReactComponent as Search} from "../assets/search.svg";

// class User extends Component {
//     constructor(props) {
//         super(props);
//         this.state = { 
//             users:[],
//             tempUsers:[],
//             search:"",
//     }}

//     componentDidMount=async ()=>{
//         const response = await UserService.getAllUsers();
//         this.setState({users:response.data})
//         this.setState({tempUsers:this.state.users})
//     }

//     handleDelete = (id) =>{
//         UserService.deleteUser(id).then(res=>{
//         const users = this.state.tempUsers.filter(user=>user.userId!==id);
//         this.setState({tempUsers:users});
//       }
//       )
//     }

//     handleSearch = (event) =>{
//       this.setState({ search: event.target.value });
//       const filteredUsers = this.state.users.filter(
//       (user) =>
//         user.role.toLowerCase().includes(event.target.value.toLowerCase())
//     );
//     this.setState({ tempUsers: filteredUsers });
//     }

//     render() { 
//         return (  
//           <div className="d-flex flex-column">
//             <div>
//             <form>
//             <Search className="float-end m-2"/>
//               <input
//                 type="search"
//                 className="form-control w-50 border float-end m-2"
//                 placeholder="Search by role"
//                 onChange={this.handleSearch}
//                 value={this.state.search}
//               />
//             </form>
//           </div>
//           <NavLink className="navbar-brand" to="/user/add">
//           <div>
//           <input
//             type="button"
//             className="btn btn-light float-end m-2 mb-4 border border-dark rounded"
//             value="Add"
//           />
//         </div>
//         </NavLink>
//             <UserTable 
//             users={this.state.tempUsers} 
//             handleDelete={this.handleDelete}
//             handleUpdate={this.handleUpdate}/>
//             </div>
//         );
//     }
// }
 
// export default User   ;